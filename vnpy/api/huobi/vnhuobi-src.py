...
def createSign(params, method, host, path, secretKey):
    ...
    sortedParams = sorted(list(params.items()), key=lambda d: d[0], reverse=False)
    # print(path, sortedParams)
    encodeParams = urlencode(sortedParams)
...
class TradeApi(object):
    ...
    def apiPost(self, path, params):
        ...
        url = self.hosturl + path + '?' + urlencode(signParams)
    ...
    def placeOrder(self, accountid, amount, symbol, type_, price=None, source=None):
        ...
        # print('inside placeorder: ' , params)
        return self.addReq(path, params, func, callback)           
    ...
    def run(self):
        ...
        while self.active:
            try:
                ...
                # result = gzip.decompress(stream).decode('utf-8')
